...
let {userId} = storeToRefs(useAuthStore());
...
<template>
  <nav class="sticky top-0 z-40 bg-[var(--p-surface-ground)] border-b border-[var(--p-surface-400)] shadow-sm">
    <div class="flex justify-between items-center gap-8 px-8 py-4 h-[70px] md:gap-4 md:px-4 md:h-[60px] sm:gap-2 sm:px-3 sm:h-14">
      ...
      <div class="hidden md:flex items-center gap-4 flex-1 justify-center">
        <RouterLink to="/" class="text-xl font-semibold no-underline text-[var(--p-text-color)] transition-colors duration-300 -tracking-[0.5px] hover:text-[var(--p-primary-color)]">
          Expenses
        </RouterLink>
      </div>
      ...
      <div class="flex items-center gap-4 flex-shrink-0 md:flex-1 md:justify-end sm:gap-2">
        ...
        <div v-else class="flex items-center gap-3 sm:gap-1">
          <span class="text-sm text-[var(--p-text-muted-color)] px-3 py-2 bg-[var(--p-surface-50)] rounded-md font-medium">{{ userId }}</span>
          <Button
            label="Logout"
            size="small"
            severity="danger"
